/* fz-cart-delegate.js — Cart (localStorage) + editable drawer
   - Capture-phase listeners + stopPropagation() to prevent overrides
   - Re-render drawer immediately after add
*/
(function(){
  if (window.__FZ_CART_WIRED__) return;
  ...
  function fmt(v, cc){ const n=parseFloat(v||0); const c=(cc||'GBP').toUpperCase(); const sym=c==='GBP'?'£':c==='USD'?'$':c==='EUR'?'€':''; return (sym||c+' ')+(isFinite(n)?n:0).toFixed(2); }
  ...
  // add-to-cart
  function shouldProcessAdd(sig){ const now=Date.now(); const last=window.__FZ_LAST_ADD__||{ts:0,sig:''}; if(last.sig===sig&&(now-last.ts)<350) return false; window.__FZ_LAST_ADD__={ts:now,sig:sig}; return true; }
  function getSelectedValue(btn){ const name=btn.dataset.optionInput||'size'; const root = btn.closest('.fz-product')||document; const el = root.querySelector(`input[name="${name}"]:checked`); return el?el.value:''; }
  ...
  function onDocClick(ev){
    ...
    if (target.closest('#fz-cart-btn')) {
      ...
      renderDrawer(); const d=$('#fz-cart-drawer'); if(d) d.hidden=false; return;
    }
    if (target.closest('[data-close="fz-cart-drawer"]')) {
      ev.preventDefault(); ev.stopPropagation();
      const d=$('#fz-cart-drawer'); if(d) d.hidden=true; return;
    }
    ...
    // 产品页：Add to Cart（阻断其它监听器）
    const addBtn = target.closest('.js-add-to-cart');
    if (addBtn) {
      ev.preventDefault(); ev.stopPropagation();
      const now = Date.now(); if (addBtn._lock && (now - addBtn._lock) < 280) return; addBtn._lock = now;
