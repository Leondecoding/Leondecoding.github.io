...
async function boot(){
  try{
    ...
    state.translations = { en: { nav: {
      home:"HOME", ourStory:"OUR STORY", blog:"BLOG", originalArt:"ORIGINAL ART",
      artPrint:"ART PRINTS", greetingCard:"GREETING CARDS", contact:"CONTACT",
      language:"LANGUAGE", login:"Login"
    }}};
  }
  try{ state.data.blog = await loadJSON("data/blog.json"); }catch(e){ state.data.blog = []; }
  document.getElementById("ytLink").href = state.socials.youtube;
  document.getElementById("igLink").href = state.socials.instagram;
...
async function renderStory(){
  ...
  section.innerHTML = `
    <div class="prose">
      ...
      <p>We also post arts and stories related to nature and life via our
        ...
        and
        <a href="\${state.socials.instagram}" target="_blank" rel="noopener">Instagram</a>.
      </p>
  ...
  form.addEventListener("submit",(e)=>{
    ...
    form.outerHTML = '<p class="desc">Thank you! You\'re on the list.</p>';
  });
}
async function
...
function renderContact(){const v=$("#view");v.innerHTML="";v.append(h("section",{},h("h1",{class:"page-title"},""),h("p",{style:"text-align:center"},"For commissions or questions, email us:"),h("p",{style:"text-align:center"},h("a",{href:"mailto:hello@example.com"},"Send Email"))))}
function renderLogin(){const v=$("#view");v.innerHTML="";v.append(h("section",{},h("h1",{class:"page-title"},"")))} boot();