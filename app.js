...
async function renderStory(){
  ...
  const wrap = h("section", {}, 
    h("div", {class:"prose"}, 
      h("p", {}, 
        "Hi, we are FZ Studio, an architectural designer and an artist, currently based in London, UK. ",
        "We enjoy creating art by meditating and experimenting through drawing, painting, photographing and crafting. ",
        "We love nature and wildlife. In our spare time, hiking always refreshes our mind for creativity."
      ),
      h("p", {}, 
        "We also post arts and stories related to nature and life via our ",
        h("a",{href: state.socials.youtube, target:"_blank", rel:"noopener"}, "YouTube"),
        " and ",
        h("a",{href: state.socials.instagram, target:"_blank", rel:"noopener"}, "Instagram"),
        "."
      ),
      h("p", {}, 
        "Currently we are introducing our first collection of watercolour greeting cards and art prints. ",
        "We believe watercolour captures the atmosphere of nature and its unpredictability makes it suitable for intuitive creation."
      ),
    )
  );

  // Newsletter
  const card = h("div", {class:"newsletter"});
  card.append(
    h("h2", {}, "NEW RELEASES & MORE"),
    h("p", {class:"desc"}, "Sign up with your email address to receive updates for new art releases, ideas from our craft and much more.")
  );
  const form = h("form", {id:"newsletterForm"});
  const first = h("input",{type:"text",name:"firstName",placeholder:"First Name",required:true});
  const last  = h("input",{type:"text",name:"lastName",placeholder:"Last Name",required:true});
  const email = h("input",{type:"email",name:"email",placeholder:"Email Address",required:true});
  const btn   = h("button",{type:"submit"},"SIGN UP");
  form.append(first,last,email,btn);
  card.append(form, h("small", {}, "100% spam-free. Unsubscribe at any time. We respect your privacy."));

  form.addEventListener("submit", (e)=>{
    e.preventDefault();
    const payload = {first:first.value.trim(), last:last.value.trim(), email:email.value.trim(), ts:Date.now()};
    try{
      const list = JSON.parse(localStorage.getItem("newsletter")||"[]");
      list.push(payload);
      localStorage.setItem("newsletter", JSON.stringify(list));
      form.replaceWith(h("p", {class:"desc"}, "Thank you! You're on the list."));
    }catch(err){
      console.error(err);
      alert("Saved locally. You can connect this to your email provider later.");
    }
  });
...
async function renderShop(kind){const v=$("#view");v.innerHTML="";const wrap=h("section",{class:"shop shop-"+kind});const title= kind==="prints" ? "Art Prints" : "Greeting Cards";const wrap=h("section",{},h("h1",{},title));const grid=h("div",{class:"gallery"});const items= state.data[kind] || [];items.forEach((it,idx)=>grid.append(h("figure",{class:"thumb",onclick:()=>openProduct(kind,items,idx)},h("img",{src:it.image,alt:it.title_en}),h("figcaption",{class:"caption"}, it.title_en))));wrap.append(grid);v.append(wrap)}
