...
// Active nav + year
document.addEventListener('DOMContentLoaded', ()=>{const y=qs('#year'); if(y) y.textContent=new Date().getFullYear(); const path=location.pathname.split('/').pop()||'index.html'; qsa('.nav a').forEach(a=>{if(a.getAttribute('href').endswith(path)) a.classList.add('active')}); updateCartBadge(); qs('#cartBtn')?.addEventListener('click', openCart);});
