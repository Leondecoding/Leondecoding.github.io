...
(function(){
  window.openProduct = function(kind, items, index){
    ...
    function it(){ return items[i]; }
    function render(){
      const item = it();
      wrap.innerHTML='';
      const title = document.createElement('h2'); title.textContent = item.title_en || (kind==='cards'?'name of card':'name of painting'); title.style.margin='0 0 12px'; title.style.fontWeight='300'; title.style.color='#999';
      const row = document.createElement('div'); row.style.display='grid'; row.style.gridTemplateColumns='1fr 1fr'; row.style.gap='20px';
      const fig = document.createElement('figure'); const img = document.createElement('img'); img.src=item.image; img.style.width='100%'; img.style.display='block'; fig.appendChild(img);
      const info = document.createElement('div');
      info.innerHTML = '<div><strong>Size:</strong> '+(item.size_text||item.size||'TBD')+'</div>' +
                       '<div><strong>Price:</strong> '+(item.price_text||item.price||'TBD')+'</div>';
      const pay = document.createElement('a'); pay.textContent='Buy via PayPal'; pay.target='_blank'; pay.rel='noopener'; pay.href=item.paypal || item.paypal_url || '#';
